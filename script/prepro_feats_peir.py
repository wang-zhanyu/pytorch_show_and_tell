...
def main(params):
    ...
    for i,img in tqdm(enumerate(imgs)):
        # load the image
        path = os.path.join(params['images_root'], img['filename'])
        if not os.path.isfile(path):
            print('{} not existed'.format(path))
            continue
        if os.path.isfile(os.path.join(dir_fc, str(img['id'])+'.npz')):
            continue
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--input_json', default="../data/peirtalk.json", help='input json file to process into hdf5')
    parser.add_argument('--output_dir', default='../data/peirtalk', help='output h5 file')
    # options
    parser.add_argument('--images_root', default=r'E:\pHD\data\peir_gross', help='root location in which images are stored, to be prepended to file_path in input json')
